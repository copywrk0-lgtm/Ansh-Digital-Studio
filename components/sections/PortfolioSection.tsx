  ...
  return <section className='work'>
    ...
    <div className='featuredEditorial'>
      <article className='featuredLead'>
        <div className='featuredImage'><Image src={lead.image} alt={`${lead.type}: ${lead.title}`} fill className='cover' quality={85} sizes='(max-width:800px) 100vw, 42vw'/></div>
        <div className='featuredCaption'><span>01 · {lead.type}</span><h3>{lead.title}</h3></div>
      </article>
      <div className='featuredRight'>
        <article className='featuredWide'>
          <div className='featuredImage'><Image src={detail.image} alt={`${detail.type}: ${detail.title}`} fill className='cover' quality={85} sizes='(max-width:800px) 100vw, 58vw'/></div>
        ...
        <div className='featuredPair'>
          {[portrait,story].map((p,i)=><article className='featuredSmall' key={p.n}>
            <div className='featuredImage'><Image src={p.image} alt={`${p.type}: ${p.title}`} fill className='cover' quality={84} sizes='(max-width:800px) 100vw, 29vw'/></div>
